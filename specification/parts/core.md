...
All names in a G4MF file MUST NOT contain the following characters: `"`, `#`, `*`, `.`, `:`, `|`, `?`, `@`, `<`, `>`, `{`, `}`, `[`, `]`, `/`, literal `\`, and literal `%`. The `\` character is reserved as the JSON escape character and may be used for escaping characters, and the character `%` is reserved for URI percent encoding, but names MUST NOT contain literal backslashes or literal percent signs as standalone characters in the decoded string. The characters `{}[]/` are reserved for use in JSON pointer paths. The characters `"#.:@` are reserved for use in runtime-determined path syntaxes and selectors across many programming languages and contexts. The characters `"*:|?<>/` and literal backslashes are forbidden due to restrictions in file systems and shells, which would otherwise be problematic when attempting to extract the contents of a G4MF file into separate files. These characters are forbidden in ALL names to avoid ambiguity and potential parsing issues when names are used in any these contexts.
