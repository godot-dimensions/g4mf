...
Additionally, names are RECOMMENDED to be unique in a case-insensitive manner. This avoids conflicts if saving named items to files on case-insensitive file systems, and avoids conflicts with Unreal Engine's case-insensitive name-in-owner behavior.

Note that within a G4MF file, items are referred to by index in a specification-defined array (see [Integer Index Identifiers](#integer-index-identifiers)), ensuring that internal references are compact, efficient, and always refer to the correct data type. For example, this design makes it impossible for a mesh's `"material"` property to refer to a node, mesh, texture, or any other non-material item. However, indices are likely to change across file edits; therefore, unique names provide a way for external references to refer to items in a more stable way.

All names in a G4MF file MUST NOT contain the following characters: `"`, `#`, `*`, `.`, `:`, `|`, `?`, `@`, `<`, `>`, `{`, `}`, `[`, `]`, `/`, literal `\`, and literal `%`. Rationale:

- The character `\` is reserved as the JSON escape character and may be used for escaping characters, and the character `%` is reserved for URI percent encoding, but names MUST NOT contain literal backslashes or literal percent signs as standalone characters in the decoded string.
