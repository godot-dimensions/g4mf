# G4MF Model File Reference

## Overview

Models are references to other self-contained files that can be used in the scene hierarchy. They can be used to reference other G4MF files, [OFF](https://en.wikipedia.org/wiki/OFF_%28file_format%29) files, glTF™ files, [OCIF](https://github.com/ocwg/spec) files, or any other format.
...
Specifying a URI is highly recommended, as it allows using an external file for the model, which is the main motivation for separating a hierarchy of nodes into their own isolated models. However, storing model data in a buffer view may still be useful for deduplicating data in cases where the same hierarchy of nodes is used many times in the same G4MF file.
...
| `model/3mf`             | `.3mf`             | 3D Manufacturing Format           | Supports multiple meshes, but no scene hierarchy. May be imported as a mesh node or multiple mesh nodes.                                                                        |
| `model/fbx` ⚠️          | `.fbx`             | Autodesk FilmBox                  | Can be directly imported as a hierarchy of nodes.                                                                                                                                    |
| `model/vnd.usda`        | `.usda`            | Universal Scene Description ASCII | Can be directly imported as a hierarchy of nodes, contain a scene hierarchy with a root "prim".                                                                                      |
| `model/vnd.usdc` ⚠️     | `.usdc`            | USD Compressed Binary             | Can be directly imported as a hierarchy of nodes, contain a scene hierarchy with a root "prim".                                                                                      |
| `model/vnd.usdz+zip`    | `.usdz`            | Universal Scene Description ZIP   | Can be directly imported as a hierarchy of nodes, contain a scene hierarchy with a root "prim".                                                                                      |
| `model/vnd.collada+xml` | `.dae`             | Khronos COLLADA™ XML              | Can be directly imported as a hierarchy of nodes.                                                                                                                                    |
| `model/vrml`            | `.wrl` & `.wrz`    | Virtual Reality Modeling Language | Can be directly imported as a hierarchy of nodes.                                                                                                                                    |
| `model/x3d+xml`         | `.x3d` & `.x3dz`   | X3D (Extensible 3D) XML           | Can be directly imported as a hierarchy of nodes.                                                                                                                                    |
| `model/x3d+vrml`        | `.x3dv` & `.x3dvz` | X3D (Extensible 3D) VRML          | Can be directly imported as a hierarchy of nodes.                                                                                                                                    |
| `model/x3d+binary`      | `.x3db` & `.x3dbz` | X3D (Extensible 3D) Binary        | Can be directly imported as a hierarchy of nodes.                                                                                                                                    |
| `application/ocif+json` | `.ocif`            | Open Canvas Interchange Format    | Can be directly imported as a hierarchy of nodes. Meant for canvases and GUIs.                                                                                                       |

⚠️ = Not registered with IANA, and may possibly change if registered in the future.

Implementations are not expected to support all of these MIME types, they may support some, one, or even none of them. This list is also not intended as suggestions for model formats to implement, since it includes deprecated formats such as COLLADA™ and VRML, and proprietary formats such as FBX. This list is intended as a non-comprehensive set of examples of formats that possibly make sense to be used as models due to their ability to have their data imported as a hierarchy of nodes in a scene hierarchy.
...
- See [g4mf.node.model_instance.schema.json](../../schema/g4mf.node.model_instance.schema.json) for how models are used on nodes.
