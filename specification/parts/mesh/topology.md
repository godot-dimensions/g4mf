...
All geometry accessors MUST be of an unsigned integer component type, and MUST have the `"vectorSize"` property undefined or set to its default value of 1. Each primitive number component in the accessor is a count of how many elements are on the boundary of a polytope, followed by that many indices of those elements. For example, `4 0 1 2 3` defines a face with 4 edges, which are the edges at indices 0, 1, 2, and 3 in the edges accessor.
