...
Node names MUST be file-unique, MUST NOT include `"`, `#`, `%`, `*`, `.`, `:`, `|`, `?`, `@`, `<`, `>`, `{`, `}`, `[`, `]`, `/`, literal `\`, and literal `%` characters, and MUST follow all of the requirements for G4MF item names as defined in [G4MF Core](core.md#name). Node names are RECOMMENDED to be PascalCase, however any compliant name is allowed.
...
Note for asset authors: Guidance on how to construct node hierarchy of a "single object" properly:

- A G4MF node with physics motion properties is referred to as a "physics body" for short. When this component is present for a object subtree, asset authors SHOULD prefer the top node of the subtree to be the physics body, and then have other nodes as children of it. This ensures that child component nodes follow the physics body when it moves, since physics bodies have their motion controlled by the physics engine.
- To add a mesh to a physics body, the node mesh property MUST be defined on a separate node, which is then used as a child of the physics body node. Similarly, other types may be placed on separate nodes. Meshes and other components cannot be placed on the same node as a physics body.
- For skeletons, each bone MUST be a child of another bone or of the skeleton itself, meaning that to attach a non-skinned mesh, light, or other component to a bone, such a node must be added as a child of the bone. Meshes and other components cannot be placed on the same node as a bone or skeleton. The skeleton node is usually the root of such a "single object" hierarchy, except for when it is also the child of a physics body. This ensures that child component nodes move with the skeleton as it animates.
- For subtrees without physics motion or skeletons, asset authors SHOULD prefer types _other than mesh or model_ as the top node. Mesh nodes are often leaf nodes, which simplifies some use cases of mesh nodes. This is not a requirement, but a recommendation to improve consistency when importing G4MF files into different applications with different requirements on tree structure.
