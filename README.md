...
- G4MF mesh surfaces have a `"polytopeSimplexes"` boolean, providing the glTF™ extension `FB_ngon_encoding` in the base specification. https://github.com/KhronosGroup/glTF/pull/1620
...
- G4MF meshes separate base mesh geometry from deformation systems. The `"blend"` and `"skin"` properties are optional deformation overlays on top of the base mesh, and are clearly separated from the base mesh data. glTF™ does not have this separation, the morph targets, joint indices, and joint weights are stored within each mesh primitive, alongside the base mesh data.
